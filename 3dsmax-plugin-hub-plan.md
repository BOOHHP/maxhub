...
建议使用 3ds Max .NET SDK 编写 Connector，并为 Max 2019 至 2026 按年份或经验证的 SDK ABI 兼容分组建立构建矩阵。Connector 是薄客户端，不存储核心安装逻辑。
...
### 阶段 3：MaxHub Connector，5-7 周

- 使用 Max .NET SDK 实现 Max 2019 至 2026 的 Connector 构建矩阵，由 Agent 统一安装。
- 完成停靠面板、Agent 通信、安装状态展示和 MacroScript 注册。
- 在 Max 2019 至 2026 各版本上进行安装、启动、通信和脚本加载兼容性回归。
...
| Max 多版本目录和 SDK 差异 | Connector 按版本构建；兼容性由 manifest 强制声明；建立真实版本回归矩阵。 |
...
5. Connector 采用逐年份构建还是 ABI 兼容分组，以及每个 Max 版本的用户级安装目录与检测依据。
