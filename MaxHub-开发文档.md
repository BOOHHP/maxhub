...
### 分类清单服务端下发（消除前端硬编码同步负担）

- 新增公开端点 `GET /api/v1/categories`，返回 `ToolCategoryClassifier.Categories`。
- 市场页侧栏与后台编辑下拉改为动态渲染该清单，原硬编码列表仅保留为加载失败兜底。
...
| 1.1.0 | 界面大改版：左侧改宽文字导航（圆角高亮块）、内容区子 tab 改下划线式、对齐深色高级感视觉（近黑分层 + 浅蓝主按钮）。2026-08-27 二次配色：将过黑的近黑背景提亮为深灰蓝分层，提升可读性（覆盖发布，版本号不变） |
...
| 1.5.13 | 反馈正文改用 Base64 走 GET 查询串，绕开 MaxScript 二进制封送缺陷；失败时弹窗显示真实异常文本 |
| 1.5.14 | 工具中心对话框支持最小化（`#style_minimizebox`） |

当前生产版本：**1.5.14**，兼容 Max 2019–2026。
...
- Agent 1.1.2 SHA-256：`df0d23c99c4f8c7dc771953e9bce4a07915050c9a344606cbe7b5e234c5ff625`。（上传页新增"我的提交"列表，待审核版本可在 Agent 内直接撤回）
...
- 完整测试 176/176 通过（Core 57、Agent 51、Server 68）。
